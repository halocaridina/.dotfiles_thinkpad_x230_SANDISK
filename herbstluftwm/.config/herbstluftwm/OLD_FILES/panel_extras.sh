...
net_widget() {
    ...
    dzen_icons="/home/srsantos/.config/herbstluftwm/icons/stlarch"
    wifi_icon="$dzen_icons/wireless5.xbm"
    echo -n "^i($wifi_icon) $ip ($ssid)"
}

volume_widget() {
    dzen_icons="/home/srsantos/.config/herbstluftwm/icons/stlarch"
    curr_volume=$(/usr/bin/pulseaudio-ctl full-status | awk '{print $1}')
    mute=`/usr/bin/pulseaudio-ctl full-status | awk '{print $2}'`
    if [ ${mute} == "yes" ]; then
        volume="M"
        vol_icon="$dzen_icons/vol3.xbm"
    else
        volume=$(/usr/bin/pulseaudio-ctl full-status | awk '{print $1}')
        vol_icon="$dzen_icons/vol4.xbm"
    fi
...
##        offset=0 #14 for the one icon im using that isnt counted
...
} | dzen2 -w $panel_width -x $x -y $y -fn "PragmataPro:size=9" -h $panel_height \
